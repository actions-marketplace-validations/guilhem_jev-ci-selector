...
const catalog = parse(readFileSync(resolve(example, '.github/ci-selector.yml'), 'utf8'));
...
if (!Object.hasOwn(jobs, 'tasks')) {
  ...
  for (const task of tasks) {
    ...
    for (const dependency of catalog.tasks[task].requires ?? []) assert.ok(taskNeeds.includes(dependency), `${task} must need ${dependency}`);
  }
} else {
  ...
  for (const task of tasks) assert.equal(catalog.tasks[task].requires, undefined, `${task} unexpectedly has a dependency`);
}
